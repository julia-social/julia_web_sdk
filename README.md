...
- For host websites/services, use the language-specific server adapter first so `/auth/*` routes, session tracking, and websocket proxying are handled consistently.
- Use `SignatureClient` directly when you need manual endpoint calls (custom flow, scripts, or tests).

## Endpoint Contract

Auth endpoints exposed by adapters:

- `GET /auth/notbot`
- `GET /auth/status`
- `POST /auth/notbot/{request_id}`
- `POST /auth/verify/{request_id}`
- `WS /auth/honestbot`
...
1. Host calls `GET /auth/notbot` on an SDK adapter.
2. Adapter calls upstream `POST /signature/start`, stores `request_id -> session_id`, and returns `request_id`.
3. Client/not.bot submits nonce to `POST /auth/notbot/{request_id}`.
4. Adapter calls upstream `POST /signature/presentation` and returns the compressed server presentation.
5. Client/not.bot submits signed presentation to `POST /auth/verify/{request_id}`.
6. Adapter calls upstream `POST /signature/verify`; on success, callback/session state is updated.
7. Host checks auth state with `GET /auth/status`.
8. Websocket routes `/auth/honestbot` and `/calculate_site_pass` are proxied to upstream signature services.
